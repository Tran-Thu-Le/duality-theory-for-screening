...
universe in which screening will later be defined: FR
...
An FR representation is a quadruple
\[
p\equiv (f,g,A,a),
\]
\end{definition}
...
For a function \(h:\mathbb R^n\to\overline{\mathbb R}\), its Fenchel
conjugate is
\[
h^*(y):=\sup_{x\in\mathbb R^n}\{\langle y,x\rangle-h(x)\}.
\]
When \(h\) is closed, proper, and convex, conjugation is an involution,
\(h^{**}=h\), a fact used repeatedly below. The FR dual applies this
conjugate to \(f\) and \(g\) separately.

The FR dual of \(p=(f,g,A,a)\) is
\[

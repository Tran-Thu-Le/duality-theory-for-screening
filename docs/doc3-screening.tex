...
equivariant pair under FR duality. We define both as transformations on
FR representations, show that each decomposes into a translation
followed by a restriction, and use this decomposition to prove the
main duality identity. As a by-product, we obtain a reduction in
intrinsic complexity and a characterization of simultaneous screening.
...
screening condition for all indices \(i\in\bar J\) can be rewritten as the following condition\footnote{The original sample screening rule in~\citep{ogawa2013} also considers the condition
\(
(1,\ldots,1)\in\partial f_{\bar J}(A_{\bar J}x).
\)
This is another special case covered by our generalized sample screening operator, but we omit it here for simplicity.}
\[
...
parameter level and the translation level. Definition~\ref{def:equivariance}
(Section~\ref{sec:operators}) defines equivariance for transformations
of \(\Gamma\); the parameter maps \(\kappa^F_{I,z},\kappa^S_{J,s}:\Gamma\to K\)
satisfy the same pattern, now paired with the dual-parameter map
\(k\mapsto k^*\) on \(K\) rather than FR duality on \(\Gamma\).
...
\((f^*)_{\bar J}(s)=f_{\bar J}^{*}(s)\), the same separability fact used
above, now applied to \(f\).
Hence
\[
...
Proposition~\ref{prop:parameter-equivariance} shows that the
feature--sample asymmetry, a domain shift and a value shift versus a
slope shift and a conjugate value shift, is fully absorbed by the
canonical translation parameters: FR duality exchanges one for the
other. The next step is to lift this parameter equivariance from
parameters to translation operators.
...
The sample case follows by an analogous argument. Since \(\kappa^S_{J,s}(p)\)
...
\(S^F_{I,z}\) and \(S^S_{I,z}\) are equivariant under FR duality
(Definition~\ref{def:equivariance}, Section~\ref{sec:operators}):
\[
...
\begin{proof}
We prove the first identity; the second follows by a symmetric argument
with the roles of \(f\) and \(g\), \(I\) and \(J\), domain and slope
